 ...
 */
export function formatPurchaseQuantity(encodedValue, mainUnit = "") {
  if (!encodedValue && encodedValue !== 0) {
    return "0" + (mainUnit ? ` ${mainUnit}` : "");
  }
  
  // For purchases, we use the full numeric value without breaking into sub-units
  const numericValue = typeof encodedValue === 'string' 
    ? parseFloat(encodedValue) 
    : encodedValue;
  ...
  // For purchase display, just show the full value with the main unit
