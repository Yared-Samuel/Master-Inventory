...
export function formatQuantityWithUnits(quantity, product) {
  ...
  const and = " and "
    ...
    if (wholeMainUnits > 0 && remainderSubUnits > 0) {
      ...
      return (<span class="bg-[#a1beda] text-[#2c6cab] text-base me-2 px-2.5 py-0.5 rounded-sm dark:bg-green-900 dark:text-green-300">{wholeMainUnits} {product.measurment_name} {and} {remainderSubUnits} {product.sub_measurment_name}</span>);
    } else if (wholeMainUnits > 0) {
      // Has only whole units
      return (<span class="bg-[#a1beda] text-[#2c6cab] text-base me-2 px-2.5 py-0.5 rounded-sm dark:bg-green-900 dark:text-green-300">{wholeMainUnits} {product.measurment_name}</span>);
    } else {
      // Has only sub-units (less than 1 main unit)
      return (<span class="bg-[#a1beda] text-[#2c6cab] text-base me-2 px-2.5 py-0.5 rounded-sm dark:bg-green-900 dark:text-green-300">{remainderSubUnits} {product.sub_measurment_name}</span>);
    }
  }