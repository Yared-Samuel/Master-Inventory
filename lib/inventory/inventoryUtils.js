...
import Sprice from "@/models/sellingPriceModel";
 ...
 */
export async function checkInventoryAvailability(productId, storeId, quantity, status = 'done') {
  try {
...
export async function checkInventoryAvailabilityToStore(productId, toStore, status = 'done') {
  try {
    const Transaction = getInventoryModel();
    const latestInventory = await Transaction.find({
      productId: productId, 
 ...
 */
export async function getSellingPrice(toStore, productId) {
  try {
    ...
    if (!store?.Sprice) {
      return { 
        success: false, 
        message: "Price not assigned to store" 
      };
    }
    ...
    if (!priceDetail?.products[0]?.sellingPrice) {
      return { 
        success: false, 
        message: "Selling price not found for this product" 
      };
    }
...
export async function getProductType(productId){
  try {
