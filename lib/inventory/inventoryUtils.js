 ...
 */
export async function checkInventoryAvailability(productId, storeId, quantity, status = 'done', companyId) {
  try {
    const { type } = await Product.findOne({ _id: productId, companyId: companyId }).select('type');    
    console.log(type)
    if(type == "forSale"){
      return {
        success: true, 
