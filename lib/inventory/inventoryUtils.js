...
export async function checkInventoryAvailability(productId, storeId, quantity, status = 'done', companyId, session = null) {
  ...
  try {
    ...
    if(type == "forSale"){
      ...
      toast.info(`Inventory check for forSale product: ${productId}, store: ${storeId}, quantity: ${quantity}, status: ${status}, companyId: ${companyId}`);
      return {
        success: success, 
        remaining: remaining,
...
export async function checkInventoryAvailabilityForSale(productId, storeId, quantity, status = 'done', companyId, session = null, customUsedProducts = null) {
  // Use a session if provided, or start a new one if checking forSale product
  let localSession = session;
  let shouldEndSession = false;
