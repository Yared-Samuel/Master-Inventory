...
export function restrictToRoles(allowedRoles = []) {
  return (handler) => {
    return async (req, res) => {
      // Check if user exists on the request (should be set by authenticateUser)
      if (!req.user) {
        console.log("Role restriction: No user found in request");
      ...
      if (allowedRoles.length > 0 && !allowedRoles.includes(userRole)) {
        console.log(`Role restriction: User role ${userRole} not in allowed roles:`, allowedRoles);
        return sendForbidden(res, "You don't have permission to access this resource");
      }
      
      // Allow access if role check passes
      console.log(`Role check passed for user ${req.user.id} with role ${userRole}`);
      return handler(req, res);
    };
  };
}
  ...
  return (handler) => {
    return async (req, res) => {
      try {
        ...
        const { getUserModel } = require("@/lib/models");
        ...
        console.log(`protectRoute: User authenticated - ID=${req.user.id}, Role=${req.user.role}`);
