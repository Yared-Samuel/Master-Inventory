  ...
  return async (req, res) => {
    try {
      ...
      req.user = {
        id: tokenData.id,
        role: tokenData.role,
        companyId: tokenData.companyId // Important: Include companyId
      };
      
      // For debugging
      console.log(`User authenticated: ID=${req.user.id}, Role=${req.user.role}, CompanyId=${req.user.companyId}`);
